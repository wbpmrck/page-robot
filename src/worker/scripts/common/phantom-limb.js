...
(function() {
	...
	function fireTouchEvents(eventName, originalEvent) {
        ...
        console.log(`phamtomlib: fireTouchEvents:${eventName}`);
		// For each finger with a target, create a touch event.
		fingers.forEach(function(finger) {
			if (!finger.target) {
                console.log(`phamtomlib: finger no target`);
				return;
            }
            console.log(`phamtomlib: fireTouchEvents->fingers:`);
            console.log(finger.node);
            ...
            console.log(`phamtomlib: fireTouchEvents->push:`);
			events.push(touch);
		});

		// Figure out scale and rotation.
		if (events.length > 1) {
            console.log(`phamtomlib: fireTouchEvents->Figure out scale and rotation:`);
		...
		events.concat(gestures).forEach(function(event, i) {
            console.log(`phamtomlib: fireTouchEvents->fire:`+event.type);
			event.identifier = i;
